...
class HomePage extends StatelessWidget {
                        ...
                        child: SingleChildScrollView(
                          ...
                          child: Column(
                            children: [
                              ...
                              const Center(child: Text('©️ 2025 - 2026 نخلاوي', style: TextStyle(color: Colors.grey, fontSize: 12))),
      ...
      child: Container(
        ...
        child: const Row(
          children: [
            ...
            SizedBox(width: 12),
            Text('ابحث عن أمراض، خبراء، أو مقالات...', style: TextStyle(color: Colors.grey, fontSize: 14)),
          ],
        ),
      ),
    );
    ...
    return GridView.count(
      ...
      children: [
        ...
        _HomeCard(
          ...
          onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const MiniWiki())),
        ),
        _HomeCard(
        ...
        _HomeCard(
          icon: Icons.history,
          title: 'سجلك الزراعي',
          onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ConsultationsPage())),
        ),
      ],
    );
  }
  ...
  Widget _buildWelcomeCard(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(colors: [AppColors.header.withOpacity(0.6), AppColors.header.withOpacity(0.3)]),
          borderRadius: BorderRadius.circular(20),
  ...
  void _goToAiDiagnosis(BuildContext context, String title) async {
    ...
    // الانتظار للحظة لضمان مزامنة حالة المستخدم من الذاكرة المحلية
    await Future.delayed(Duration.zero);
    final user = supabase.auth.currentUser;

    if (user == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('الرجاء تسجيل الدخول أولاً لتتمكن من استخدام التشخيص'), backgroundColor: Colors.red),
      );
      return;
    }
    ...
    showDialog(
      ...
      builder: (context) => const Center(child: CircularProgressIndicator(color: AppColors.darkBrown)),
    );
    ...
    try {
      ...
      if (context.mounted) {
        ...
        Navigator.push(context, MaterialPageRoute(builder: (context) => AiConsultationDetailsPage(title: title, sessionId: sessionId)));
      }
    } catch (e) {
      if (context.mounted) {
        Navigator.pop(context);
  ...
  Widget build(BuildContext context) {
    return Material(
      ...
      child: InkWell(
        ...
        borderRadius: BorderRadius.circular(20),
        child: Container(
          decoration: BoxDecoration(color: AppColors.header.withOpacity(0.4), borderRadius: BorderRadius.circular(20)),
          child: Column(
